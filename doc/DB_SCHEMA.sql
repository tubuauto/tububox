...
CREATE UNIQUE INDEX uniq_api_keys_key ON api_keys(api_key);
...
CREATE INDEX idx_deliveries_source_order_no ON deliveries(source_order_no);
CREATE INDEX idx_deliveries_created_at ON deliveries(created_at);

CREATE UNIQUE INDEX uniq_deliveries_tenant_idempotency
...
CREATE INDEX idx_proof_of_delivery_delivery_id ON proof_of_delivery(delivery_id);
...
CREATE INDEX idx_cod_collections_delivery_id ON cod_collections(delivery_id);
...
CREATE INDEX idx_webhook_logs_endpoint_id ON webhook_logs(webhook_endpoint_id);
